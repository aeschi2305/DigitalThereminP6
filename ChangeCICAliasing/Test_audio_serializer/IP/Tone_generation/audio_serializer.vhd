...
architecture rtl of audio_serializer is
  ...
  type streaming_fifo is array (0 to 1) of std_logic_vector (23 downto 0);
  signal audio_cmb            : std_logic_vector(23 downto 0);
  signal audio_reg            : std_logic_vector(23 downto 0);
  signal streaming_data       : streaming_fifo;
  ...
  begin
    if reset = '1' then
      ...
      if emptied = true then
        if aso_seR_valid = '1' then
          streaming_data <= streaming_data(0) & aso_seL_data;
          aso_seR_ready <= '0'
        end if;
      end if;
    end if;
